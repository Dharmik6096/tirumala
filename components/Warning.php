...
class Warning extends Component {
    ...
    public function unique($model, $field, $value, $msg = '', $custMsg = '') {
        ...
        if ($values != 0) {
            ...
            $message = !empty($custMsg) ? $custMsg : \Yii::t('app', $value . " has been already taken. Are you sure you want to continue?");
    ...
    public function unique_member($model, $fields, $values, $msg = '') {
        ...
        $primaryKey = $model->tableSchema->primaryKey[0];
        $value = $model->find()->where([$fields[0] => ucwords($values[0]), $fields[1] => ucwords($values[1]), $fields[2] => ucwords($values[2]), 'is_active' => 1])->andWhere(['<>', $primaryKey, $model->$primaryKey]);
