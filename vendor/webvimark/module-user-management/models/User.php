...
//        public $user_type;
	...
	public static function getUserTypeList()
	{
            ...
            $userType = \yii\helpers\ArrayHelper::map($userType, function($array,$key){return $array['id'].'-'.$array['user_type']; }, 'user_type');
...
//			['username', 'unique'],
			...
			[['federation','mobile_no','alert_recipient_group_id','user_identity','union','dcs','organizations','user_type_id','role','created_by','deleted_by','updated_by','flg_sentbox_entry','sync_status','sync_timestamp','portal_type'], 'safe'],
                        ['bind_to_ip', 'trim'],
    ...
    public static function getSelectedOrganization($value){
             ...
             switch ($value) {
                 ...
                 case '4' :   $unionModel=new TblUnions();
                              $union=$unionModel->getUnion(Yii::$app->session->get('Federations'));
                              $union_code=implode(',', array_map(function($a) {return $a['union_code'];},$union));                              
                              $model=new TblDcs();
                              $data=$model->getDcs($union_code);
                              $field[0]='dcs_code';
                              $field[1]='dcs_name';
                              break;
                 case '3' :   $model=new TblUnions();
                              $data=$model->getUnion(Yii::$app->session->get('Federations'));
                              $field[0]='union_code';
                              $field[1]='union_name';
                              break;
                 case '2' :   $model=new TblFederations();
                              ...
                              break;
                 case '1' :   
                              //$model=new \app\models\TblNational();
