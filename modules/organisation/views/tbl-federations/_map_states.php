...
$title = Yii::$app->label->title('create', 'State');
$button = Yii::$app->label->button('create');

$this->title = Yii::t('app', $title);
?>
<div class="panel panel-default panel-main">
    <div class="panel-heading">States for <?= $federation_name ?></div>
    <div class="panel-body">
        ...
        <div class="row">
            ...
            <div class="col-sm-12">
                <?php
                echo $form->field($model, 'state_code[]')->checkboxList(
                        $state_list, [
                    'id' => 'routes-list',
                    'class' => 'row mb15',
                    'item' =>
                    function ($index, $label, $name, $checked, $value) use ($selected, $defaultValue, $modelFederation) {
                        $checked = in_array($value, $selected);
                        $modelFederation->federation_code = Yii::$app->getRequest()->getQueryParam('id');
                        $modelFederation->state_code = $value;
                        $checkChild = $modelFederation->getCheckUnionExist();
