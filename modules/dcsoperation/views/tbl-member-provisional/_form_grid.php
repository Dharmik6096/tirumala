    ...
    'actions' => [
        'update' => function ($url, $model)use ($pending_approval) {
            if (Yii::$app->general->getUnionConfiguration(explode(',', Yii::$app->session->get('Unions')), 'workflow_require', 'PORTAL') == 0) {
                ...
                $class = '';
                if (!$pending_approval) {
                    $class = ($model->is_active === 0 || $model->provisional_status != 'Pending' || $model->provisional_status != 'Re-route') ? 'link-disable' : '';
                }
                $name = $model->member_name;
        ...
        'document-upload' => function ($url, $model) use ($pending_approval) {
            if (!$pending_approval) {
                $disable = ($model->provisional_status == 'Pending' || $model->provisional_status == 'Re-route') ? '' : 'disabled';
        ...
        'report' => function ($url, $model) use ($pending_approval) {
            if (!$pending_approval) {
                $disable = in_array(strtolower($model->provisional_status), ['approve', 'register', 'inprogress', 'pending', 'reject', 'Re-route']) ? '' : 'disabled';
