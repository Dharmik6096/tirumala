...
$script = "
    ...
    $('#tblpurchaseratebased-0-rate_type').on('change',function(e){
        ...
        $.each(rateType.split('+'), function(index, item)
        {
           ...
           $('#range').append(field_before + '<label class=\"control-label\">' + item + ' Start*</label><input type=\"text\" name=\"TblPurchaseRateBased['+index+'][start_range]\" class=\"form-control number-validate\">' + field_after);
           $('#range').append(field_before + '<label class=\"control-label\">' + item + ' End*</label><input type=\"text\" name=\"TblPurchaseRateBased['+index+'][end_range]\" class=\"form-control number-validate\">' + field_after);
           
           $('#range').append('<input type=\"hidden\" value=\"'+param+'\" name=\"TblPurchaseRateBased['+index+'][quality_param_code]\" >');
        });
        $.each(rateType.split('+'), function(index, item)
        {             
           $('#range').append(field_before + '<label class=\"control-label\">Kg' + item + '*</label><input type=\"text\" name=\"TblPurchaseRateBased['+index+'][kg_rate]\" class=\"form-control number-validate\">' + field_after);           
        });
    });
