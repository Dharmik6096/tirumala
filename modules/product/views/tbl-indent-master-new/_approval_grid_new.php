        ...
        $attribute = [
            ['class' => 'kartik\grid\CheckboxColumn',
                ...
                'checkboxOptions' => function($model, $key) {
                    ...
                    return ['class' => 'checkbox group-checkbox child-checkbox '.$id, 'disabled' => $disabled, 'data-id' => $id, 'id' => 'tblindentmaster-' . $key . '-process_approval_code', 'value' => $model['process_approval_code']];
                }],
            ...
            ['attribute' => 'approve_qty', 'filter' => FALSE,
                ...
                'value' => function ($model) use ($form, $indentMaster) {
                    ...
                    if (empty($level)) {
                        ...
                        return $form->field($indentMaster, '[' . $model['process_approval_code'] . ']approve_qty')->textInput(['value' => $model['qty'], 'class' => 'form-control number-validate integer-input approve_qty',  'data-id' =>$id, 'disabled' => $disabled])->label(FALSE);
                    } else {
            ...
            ['attribute' => 'amount', 'filter' => FALSE, 'visible' => ($visible ? TRUE : FALSE),
                ...
                'value' => function ($model) use ($form, $indentMaster) {
                    $id = $model['dcs_code'].$model['member_code'].$model['product_code'];
                    return $form->field($indentMaster, '[' . $model['process_approval_code'] . ']amount')->textInput(['value' => $model['amount'], 'class' => 'form-control number-validate '.$id, 'readonly' => TRUE])->label(FALSE);
                },
            ],
