...
$this->title = Yii::t('app', 'DCS wise billing mapping');
$defaultToggle = true;
?>
            ...
            <div class="grid-search search-filter padding_left_0 padding_right_0 searchBtnReport text-right beforeGridLoad">
                <?php if (!empty($model)) { ?>
                    ...
                    <div class="col-sm-2 padding-left-0 text-left">
                        <?= Yii::$app->dropdown->dropdownStatic('billing_based_on', $searchModel, $form, 'form-group padding-right-5', $searchModel->getAttributeLabel('billing_based_on'), false, 'billing_based_on') ?> 
                    </div>
                <?php } ?>
                                    ...
                                    <tbody>
                                        <?php for ($i = 0; $i < $first_table; $i++) { ?>
                                            <tr>
                                                ...
                                                <td><?= $model[$i]->dcs_name . '(' . $model[$i]->dcs_code_ex . ')'; ?><?= Html::activeHiddenInput($saveModel[$i], '[' . $i . ']billing_based_on', ['class' => 'setWefDate']) ?></td>
                                            </tr>
                                        <?php } ?>
                                    </tbody>
                                </table>
...
$script = "
    ...
    $(document).on('change', '#tblvsppaymentconfig-billing_based_on', function(){
    ...
    $(document).ready(function () {
        ...
        if(lengthOfUl > 0) {
            $('.error-summary ul li:eq(0)').before('<li>" . Yii::t('app', 'DCS Wise Billing is Already Mapped for following.') . "</li>');
        }
    });
