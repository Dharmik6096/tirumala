...
class TblPartyPaymentSearch extends TblPartyPayment {
    ...
    public function rules() {
        return [
            ...
            [['bank_account_no', 'beneficiary_name', 'is_verified', 'payment_date', 'status', 'disburse_amount', 'disburse_date', 'utr_no', 'payment_cycle_code'], 'safe'],
            ...
            [['party_master_code', 'payment_type', 'payment_cycle_code'], 'required', 'on' => 'disburse']
        ];
    }
...
//        $this->load($params);
        ...
        if(!empty($this->party_master_code))
        {
             $query->andFilterWhere(['=', 'cast(tbl_party_payment.party_master_code)', $this->party_master_code]);
        }
        if (!empty($this->from_date)) {
