...
class TblPaymentTransactionSearch extends TblPaymentTransaction {
    ...
    public function rules() {
        return [
                ...
                [['plant_code', 'from_date', 'to_date'], 'required']
        ];
    }
    ...
    public function searchRejectReinitiate($params) {
        ...
        $query->andFilterWhere(['>=', 'cast(payment_date as date)', $this->from_date]);

        $this->to_date = !empty($this->to_date) ? date('Y-m-d', strtotime($this->to_date)) : date('Y-m-d');
        $query->andFilterWhere(['<=', 'cast(payment_date as date)', $this->to_date]);
