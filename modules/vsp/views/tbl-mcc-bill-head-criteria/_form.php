...
$script = "
    ...
    gridChange();
        if(action == 'edit'){
            ...
            $('.QltyParamDiv').show();
            $('#tblmccbillheadcriteriaslabs-from_val').focus();
        }
    $(document).on('change', '#tblmccbillheadcriteria-criteria_name', function() {  
        gridChange();
    });
    $(document).on('change', '#tblmccbillheadcriteria-mcc_bill_head_code', function() {
       gridChange();
    });
    $(document).on('change', '#tblmccbillheadcriteria-general_formula_code', function() { 
         gridChange();
            var formulaCode = $('#tblmccbillheadcriteria-general_formula_code').val();
            if(formulaCode !=''){
                $('#tblmccbillheadcriteriaslabs-general_formula_code').val(formulaCode);
                $('#tblmccbillheadcriteriaslabs-general_formula_code').trigger('change');
                $('#tblmccbillheadcriteriaslabs-general_formula_code').trigger('select2:select');
                $('#tblmccbillheadcriteriaslabs-general_formula_code').on('depdrop.afterChange', function(event, id, value, jqXHR, textStatus) {
                    $('#tblmccbillheadcriteriaslabs-general_formula_code').val(formulaCode);
                    $('#tblmccbillheadcriteriaslabs-general_formula_code').trigger('change');
                    $('#tblmccbillheadcriteriaslabs-general_formula_code').trigger('select2:select');
                });
    ...
    function gridChange(){
       ...
       $('#bill-head-criteria-from .reset_field input').val('');
        ...
        var name = $('#tblmccbillheadcriteria-criteria_name').val();
        var head = $('#tblmccbillheadcriteria-mcc_bill_head_code').val();
        var formula = $('#tblmccbillheadcriteria-general_formula_code').val();
...
//                        $('#pageloader').show();
                    ...
                    success: function(data) {
                        ...
                        $('#tblmccbillheadcriteria-from_val').focus();
//                        $('#loadercontent').hide();
//                        $('#pageloader').hide();
                    },
                });
        ...
        $('#formula-submit').on('click',function(e){
            if($('#replace_value').val()==''){
                ...
                if(tmp1.length==tmp2.length){    
                    ...
                    removeError(); 
                    $('#tblmccbillheadcriteriaslabs-formula_with_val').val(formula);
...
//                                $.pjax.reload({container: '#member-grid'});
    ...
    function getFromVal(){
        $('#tblmccbillheadcriteriaslabs-from_val').val('0');
         var code = $('#tblmccbillheadcriteria-criteria_code').val();
            if(code !=''){
                $.ajax({
                    type: 'post',
                    ...
                    success: function(data) {                                        
                        var obj = $.parseJSON(data);
                         $('#tblmccbillheadcriteriaslabs-from_val').val(obj.toFixed(2));
...
";
$this->registerJs($script, View::POS_END, 'mcc-panel-before-hide-form');
?>