 ...
 */
$title = Yii::$app->label->title('create', 'Dcs/Subcenter Mapping');
$button = Yii::$app->label->button('create');

$this->title = Yii::t('app', $title);
?>
<div class="panel panel-main">
    <div class="panel-heading">Dcs/Subcenter Mapping for <?= $headLoad->criteriaTypeCode->criteria_name ?></div>
    <?php
    ...
    <div class="panel-body">
        <div class="panel-subheading">
            ...
            <div class="row">
                <?= Yii::$app->controls->date($model, $form, 'wef_date', 'form-group col-sm-3', false, date('d-m-Y')); ?>

                <div class="col-sm-3">
                    <div class="checkbox">
                        <?= $form->field($model, 'organization')->radioList([0 => 'Dcs', 1 => 'Sub Center'])->label(false); ?>
                    </div>
                </div>

                <div class="clearfix"></div>

                <?php echo $form->field($model, 'route_code', [ 'options' => ['class' => 'form-group col-sm-6',]])->listBox($routes, ['multiple' => 'multiple', 'size' => '10', 'options' => $selectedRoutes]); ?>

                <?php echo $form->field($model, 'dcs_code', [ 'options' => ['class' => 'form-group col-sm-6',]])->listBox($selectedAllOrg, ['multiple' => 'multiple', 'size' => '10', 'options' => $selectedOrganization])->label('Organization'); ?>
            </div>
        </div>
    </div>
...
<?php
$script = "
    $('#tblheadloadapplicability-route_code').on('change',function(){          
             var route = []; 
             $('#tblheadloadapplicability-route_code :selected').each(function(i, selected){ 
                route[i] = $(selected).val(); 
              });
             var org = $('input[name=\'TblHeadLoadApplicability[organization]\']:checked').val();
            $.ajax({
                        type: 'post',
                        url: '" . Url::to(['/vsp/tbl-head-load/get-dcs']) . "',    
    ...
    $('input[name=\'TblHeadLoadApplicability[organization]\']').on('change',function(){ 
            $('#tblheadloadapplicability-dcs_code').empty();
            $('#tblheadloadapplicability-route_code option').attr('selected',false);
    });
";
