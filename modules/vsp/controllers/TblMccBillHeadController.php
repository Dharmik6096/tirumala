...
use app\modules\vsp\models\TblMccBillHeadSearch;
use yii\web\Controller;
use yii\web\NotFoundHttpException;
use yii\filters\VerbFilter;
use app\modules\vsp\models\TblMccBillHeadHistory;
use app\modules\vsp\models\TblMccBillHeadApplicability;
use app\modules\vsp\models\TblMccBillHeadApplicabilityHistory;
 ...
 */
class TblMccBillHeadController extends \app\controllers\ChildController {
    ...
    public function actionIndex() {
        $searchModel = new TblMccBillHeadSearch();
    ...
    public function actionCreate() {
        ...
        $this->model = new TblMccBillHead();
        $this->viewFile = 'create';

        if (Yii::$app->request->post()) {
            $this->model->load(Yii::$app->request->post());

            $this->model->mcc_bill_head_code = (string) Yii::$app->general->getCodeAutoIncrement($this->model);
    ...
    public function actionUpdate($id) {
        ...
        if (Yii::$app->request->post()) {
            $master = [];
            $historyModel = new TblMccBillHeadHistory();
    ...
    protected function findModel($id) {
        if (($model = TblMccBillHead::findOne($id)) !== null) {
            return $model;
        } else {
    ...
    public function actionBillHeadApplicability($id) {
        ...
        $appModel->model = new TblMccBillHeadApplicability();
        $value = [];
        $value['BMC'] = 'BMC';
        ...
        $appModel->field_name = 'mcc_bill_head_code';
        $appModel->field_value = $id;
        $appModel->options = ['tanker_rate'];
        $appModel->mcc_field_name = 'applicable_code';
        $appModel->trans_label = Yii::t('app', 'MCC bill head applicabilities');
...
//        }
        $appModel->actions = ['delete' => ['option' => 'mcc_bill_head_applicabilty_code,mcc_bill_head_applicabilty_code,tbl-mcc-bill-head/delete-applicability']];
//        $appModel->dcs_filters = ['society' => 'Society', 'routes' => 'Routes', 'mcc' => 'MCC'];
    ...
    public function actionDeleteApplicability() {
        $model = TblMccBillHeadApplicability::find()->where(['mcc_bill_head_applicabilty_code' => Yii::$app->request->post('id')])->one();
        $localHistory = new TblMccBillHeadApplicabilityHistory();
    ...
    public function actionSaveApplicability() {
        ...
        $saveModel = [];
        if (!empty($data['mcc_bill_head_code'])) {
            foreach ($data['mcc_bill_head_code'] as $key => $code) {
                $model = new TblMccBillHeadApplicability();
                $model->setAttributes($data);
                $model->mcc_bill_head_code = $code;
