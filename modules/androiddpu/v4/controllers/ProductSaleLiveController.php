...
class ProductSaleLiveController extends RestController {
    ...
    public function actionAddProductSale() {
        ...
        if (!empty($data['organization_code']) && !empty($data['organization_type']) && in_array($data['organization_type'], ['VLC']) && !empty($content['product_sale']) && !empty($content['product_sale_transaction'])) {
            $master = $content['product_sale'];
            $txn = $content['product_sale_transaction'];
