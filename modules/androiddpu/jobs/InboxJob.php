...
class InboxJob extends BaseObject implements JobInterface {
    ...
    public function execute($queue) {
        try {
            if (!empty($this->transaction_data['uuid'])) {
                ...
                if ($transaction !== 'customRedirect') {
                    ...
                    if (strstr(strtolower($errorData), 'cannot insert duplicate key')) {
                        \Yii::info("AMCS Inbox Queue Parse : Error " . $this->transaction_data['uuid'] . " : " . $errorData);
                        return true;
                    } else {
                        \Yii::info("AMCS Inbox Queue Parse Refill : Re-processing UUID-" . $this->transaction_data['uuid'] . " : " . $errorData);
                        $queue->push(new self([
                    ...
                    \Yii::info("AMCS Inbox Queue Parse : Success " . $this->transaction_data['uuid']);
                }
            } else {
                \Yii::info("AMCS Inbox Queue Parse Refill : uuid missing");
                $queue->push(new self([
                    'transaction_data' => $this->transaction_data,
                    'sync_timestamp' => $this->sync_timestamp,
                ]));
            ...
            \Yii::info("AMCS Inbox Queue Parse Refill : Exception : " . $this->transaction_data['uuid'] . " : " . $e->getMessage());
            $queue->push(new self([
                'transaction_data' => $this->transaction_data,
                'sync_timestamp' => $this->sync_timestamp,
            ]));
