...
<div class=" no-effect">
    ...
    <div id="fixed-table-container" class="static_header_grid dynamic_report_table overflow_auto sticky-footer-panel fixed-table-container">
        <table id="trip_data_grid" class="table table-striped table-input" id="table">
            <?php
            if (!empty($output)) {
                $show_column = ['milk_vehicle_entry_code', 'plant_code', 'grn_no', 'vehicle_entry_date', 'qty', 'gross_weight', 'tare_weight', 'vehicle_code', 'current_trip_code', 'trip_code'];
                ?>
                <thead>
                    <tr>
                        ...
                        foreach ($output[0] as $att => $value) {
                            if (in_array($att, $show_column)) {
                                $class = '';
                                if ($att == 'milk_vehicle_entry_code') {
                                    $class = ' disp_none';
                                }
                                ?>
                        ...
                        <tr>
                            ...
                            foreach ($value_row as $value_key => $value) {
                                if (in_array($value_key, $show_column)) {
                                    ...
                                    $class .= ' ' . $value_key . '-' . $i;
                                    if ($value_key == 'milk_vehicle_entry_code') {
                                        $class .= ' disp_none';
                                    }
...
$script = '
    $(".edit_record").click( function(){
        ...
        var milk_vehicle_entry_code_value = $(".milk_vehicle_entry_code-"+row_number).html();
        var milk_vehicle_entry_code_append = "<input type=\'hidden\' name=\'milk_vehicle_entry_code[]\' value=\'"+milk_vehicle_entry_code_value+"\'> <span>"+milk_vehicle_entry_code_value+"</span>"
        $(".milk_vehicle_entry_code-"+row_number).html(milk_vehicle_entry_code_value);
                     
        var trip_code_value = $(".trip_code-"+row_number+" #dropdown_value").val();
        var data_append_trip_code = "<select class=\'dd_trip_code\' id=\'dd_trip_code-"+row_number+"\' name = \'trip_code["+milk_vehicle_entry_code_value+"][]\'><option selected=\'true\' disabled=\'disabled\'>Select Trip Code</option>"
        $("#loadercontent").show();
        $("#pageloader").show();
        $.ajax({
            type: "post",
            url: "' . Url::to(['/tankermovement/tbl-milk-vehicle-entry/get-trip-code']) . '",
            data: {milk_vehicle_entry_code: milk_vehicle_entry_code_value},
 ...
 $(".save_record").click( function(){
        ...
        $.ajax({
            type: "post",
            url: "' . Url::to(['/tankermovement/tbl-milk-vehicle-entry/change-trip-code']) . '",
            data: {trip_code:trip_code_value,milk_vehicle_entry_code: milk_vehicle_entry_code_value},
            success: function(data) {
                var obj = data; 
                $("#loadercontent").hide();
                $("#pageloader").hide();
...
';
$this->registerJs($script, View::POS_END, 'receipt-trip-edit-script');
?>
