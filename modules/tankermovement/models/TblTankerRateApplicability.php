...
//        echo $this->wef_date.'- '.$this->dcs_code.'<br>';
    ...
    public function getReferenceCode() {
        return $this->hasOne(TblTankerRate::className(), ['reference_code' => 'reference_code']);
    }
...
//
//        $purchaseModel = TblTankerRate::findOne($this->purchase_rate_code);
//        $sentbox->setSentbox($purchaseModel, 'INSERT');
//
//        $purchaseBaseModel = TblTankerRateBased::find()->where(['purchase_rate_code' => $this->purchase_rate_code])->all();
//        foreach ($purchaseBaseModel as $model) {
//            $sentbox->setSentbox($model, 'INSERT');
//        }
//        $purchaseDetail = TblTankerRateDetails::find()->where(['purchase_rate_code' => $this->purchase_rate_code])->all();
...
//    }
    ...
    public function getTankerRateApplicableData($data) {
        return $this->find()
                        ->select(['dprd.rate_type_code as rate_app_code', 'tbl_tanker_rate_applicability.purchase_rate_code'])
                        ->joinWith(['purchaseRateCode'])
                        ->join('LEFT JOIN', 'tbl_tanker_rate_details dprd', 'dprd.purchase_rate_code = tbl_tanker_rate_applicability.purchase_rate_code AND dprd.milk_type_code =' . $data['milk_type'] . ' AND dprd.milk_quality_type_code =' . $data['milk_quality_type'] . ' AND dprd.rate_class =\'' . $data['rate_class'] . '\'')
                        ->where(['tbl_tanker_rate_applicability.is_active' => 1, 'tbl_tanker_rate_applicability.dcs_code' => $this->dcs_code, 'tbl_tanker_rate.shift_applicability' => [3, $data['shift']]])
                        ->andWhere(['<=', 'tbl_tanker_rate_applicability.wef_date', $this->wef_date])
//                        ->andWhere(['dprd.milk_type_code' => $data['milk_type'], 'dprd.milk_quality_type_code' => $data['milk_quality_type']])
                        ->orderBy('tbl_tanker_rate_applicability.wef_date desc')
                        ->one();
    }

    public function getPendingApplicability($device_id, $hash_key, $dcs_code) {
        return $this->find()->select(['tbl_tanker_rate_applicability.*'])
                        ->leftJoin('tbl_rate_download_ack', "tbl_rate_download_ack.rate_app_code=tbl_tanker_rate_applicability.rate_app_code  AND tbl_rate_download_ack.device_id='$device_id' AND tbl_rate_download_ack.hash_key='$hash_key' AND tbl_rate_download_ack.applicable_for='MEMBER'")
                        ->where(['tbl_tanker_rate_applicability.purchase_rate_code' => $this->purchase_rate_code, 'tbl_tanker_rate_applicability.dcs_code' => $dcs_code])
    ...
    public function setChildTable(&$model, &$modelSave, &$errors) {
        ...
        if (strtoupper($model->applicable_for != 'DCS')) {
            ...
            if (!empty($model->dcs_purchase_rate_code) && empty($model->purchase_rate_code)) {
                $memberRateModel = new TblTankerRate();
                $data = $memberRateModel->getDcsTankerRateCode($model->dcs_purchase_rate_code);
                if (!empty($data)) {
                    $model->purchase_rate_code = $data->purchase_rate_code;
                    $purchaseModel = new TblDcsTankerRateApplicabitity();
    ...
    public function setImport($attribute, $params) {
        if (empty($this->getErrors())) {
            ...
            if (strtolower($this->applicable_for) != 'dcs' && empty($this->dcsTankerRate)) {
...
//            }
            $purchase = new TblTankerRate();
            $code = $purchase->getValidTankerRate($this->purchase_rate_code);
...
//            $this->shift_code = 1;
    ...
    public function getDcsTankerRate() {
        return $this->hasOne(TblDcsTankerRate::className(), ['purchase_rate_code' => 'dcs_purchase_rate_code']);
    }
    ...
    public function getdispatchTankerRateApplicableData($data) {
        return $this->find()
                        ->select(['dprd.rate_type_code as rate_app_code', 'tbl_tanker_rate_applicability.purchase_rate_code'])
                        ->joinWith(['purchaseRateCode'])
                        ->join('LEFT JOIN', 'tbl_tanker_rate_details dprd', 'dprd.purchase_rate_code = tbl_tanker_rate_applicability.purchase_rate_code AND dprd.milk_type_code =' . $data['milk_type'] . ' AND dprd.milk_quality_type_code =' . $data['milk_quality_type'] . ' AND dprd.rate_class in' . $data['rate_class'])
                        ->where(['tbl_tanker_rate_applicability.is_active' => 1, 'tbl_tanker_rate_applicability.dcs_code' => $this->dcs_code, 'tbl_tanker_rate.shift_applicability' => [3, $data['shift']]])
                        ->andWhere(['<=', 'tbl_tanker_rate_applicability.wef_date', $this->wef_date])
//                        ->andWhere(['dprd.milk_type_code' => $data['milk_type'], 'dprd.milk_quality_type_code' => $data['milk_quality_type']])
                        ->orderBy('dprd.rate_class ASC,tbl_tanker_rate_applicability.wef_date desc')
                        ->one();
    }

    public function getApplicability($rate_id, $dcs_code, $date) {
        return $this->find()
                        ->select(['tbl_tanker_rate_applicability.*'])
                        ->where(['tbl_tanker_rate_applicability.is_active' => 1, 'tbl_tanker_rate_applicability.dcs_code' => $dcs_code, 'tbl_tanker_rate_applicability.purchase_rate_code' => $rate_id])
                        ->andWhere(['cast(tbl_tanker_rate_applicability.wef_date as date)' => $date])
                        ->one();
    }
    ...
    public function getDcsApplicability($dcs_code, $date) {
        return $this->find()
                        ->select(['tbl_tanker_rate_applicability.*'])
                        ->where(['tbl_tanker_rate_applicability.is_active' => 1, 'tbl_tanker_rate_applicability.dcs_code' => $dcs_code])
                        ->andWhere(['<=', 'tbl_tanker_rate_applicability.wef_date', $date])
                        ->orderBy('tbl_tanker_rate_applicability.wef_date desc')
                        ->one();
    }
