...
//        $qualityModel = new TblQualityParam();
    ...
    public function actionCreateRate($id, $method) {
        ...
        if (Yii::$app->request->post() && isset(Yii::$app->request->post()['TblTankerRateBased'])) {
            $modelAttributesLoaded = Model::createMultiple(TblTankerRateBased::classname(), [], null);
            ...
            if (!$validate) {
                ...
                if ($method == 2) {
                    //TblTankerRateBased::deleteAll(['purchase_rate_code' => Yii ::$app->request->get('id'), 'milk_type_code' => $modelAttributesLoaded[0]['milk_type_code']]);
                    $data = TblTankerRateBased::find()->where(['purchase_rate_code' => Yii ::$app->request->get('id'), 'milk_type_code' => $modelAttributesLoaded[0]['milk_type_code']])->all();
...
//        }
    ...
    /**
     * Finds the TblTankerRateDetails model based on its primary key value.
     * If the model is not found, a 404 HTTP exception will be thrown.
     * @param integer $id
     * @return TblTankerRateDetails the loaded model
    ...
    public function actionDelete() {
        ...
        try {
            $master = [];
            $record = TblTankerRateBased::findOne(Yii::$app->request->post('id'));
            $master = $this->deleteBased($record, $master);
            // $this->purchaseModel = TblTankerRate::findOne($record->purchase_rate_code);
    ...
    private function deleteBased($record, $master) {
        ...
        $data = TblTankerRateBased::find()->where('purchase_rate_code=\'' . $purchaseratecode . '\' and milk_type_code=\'' . $milktype . '\' and quality_param_code=\'' . $qualityparam . '\' and fixed_point between \'' . $startrange . '\' and \'' . $endrange . '\' ')->all();
        for ($i = 0; $i < count($data); $i++) {
            $record = TblTankerRateBased::findOne($data[$i]->rate_based_code);
            $master = $this->deleteBased($record, $master);
        }
        return $master;
    }
